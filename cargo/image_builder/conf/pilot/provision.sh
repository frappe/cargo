...
# Pilot asks this provider where a custom domain routes. The zone and the proxy subnet
# are baked in below, because neither changes for a VM of this image.
cat > "$PROVIDER_SCRIPT" <<'PROVIDER'
#!/usr/bin/env python3
...
sed -i \
	-e "s|__WILDCARD_DOMAIN__|*.$WILDCARD_DOMAIN|" \
	-e "s|__PROXY_SUBNET__|$PROXY_SUBNET|" \
	"$PROVIDER_SCRIPT"
