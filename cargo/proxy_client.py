...
class ProxyClient:
	...
	def map_domain(self, domain: str, address: str) -> None:
		"""Map one domain below the regional wildcard to a mesh address."""
		...
		try:
			...
			raise ProxyError(f"Unable to map {domain}: {exception}") from exception

		if not response.ok:
			detail = (response.text.strip() or "no response body")[:500]
			raise ProxyError(f"Unable to map {domain}: Proxy answered {response.status_code}: {detail}")
